...
class KnowledgeBaseLoader:
    ...
    def create_k8_qa_pairs_table(self):
        """Create k8_qa_pairs table with larger text fields"""
        with self.engine.connect() as conn:
            # Drop table if exists
            conn.execute(text("DROP TABLE IF EXISTS k8_qa_pairs"))
            
            # Create table with larger text fields
            create_table_sql = """
            CREATE TABLE k8_qa_pairs (
                ...
                with self.engine.connect() as conn:
                    for row in csv_reader:
                        ...
                        record = {
                            ...
                            'answer': row['answer'],  # Original answer # Normalize difficulty
                        }
                        
                        # Insert record
                        insert_sql = text("""
                            INSERT INTO k8_qa_pairs 
                            (content, question, answer)
                            VALUES (:content, :question, :answer)
                        """)
    ...
    def verify_data(self):
        ...
        with self.engine.connect() as conn:
            # Check total count
            result = conn.execute(text("SELECT COUNT(*) FROM k8_qa_pairs"))
...
if __name__ == "__main__":
    ...
    csv_path = "/home/kayden/Desktop/python_projects/ExamPrepAgent/k8s_qa.csv"  # Update this path as needed
